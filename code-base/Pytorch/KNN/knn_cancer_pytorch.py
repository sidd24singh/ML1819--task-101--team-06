...
df1=pd.read_csv('/Users/siddyboy/Downloads/ML1819--task-101--team-06-master/Assignment_2/Data_Files/Cancer_clean.csv', sep=',')
y = df1['Diagnosis']
X=df1.drop(['Diagnosis','ID_number'],axis=1)
...
x_train=x_train.view(-1,21)
y_train = Variable(torch.Tensor(y_train))

x_valid = Variable(torch.Tensor(x_valid))
x_valid=x_valid.view(-1,21)
...
plt.title('Confusion Matrix  knn Pytorch Validation Data Small Dataset', size = 12)
plt.colorbar()
tick_marks = np.arange(2)
plt.xticks(tick_marks, ["benign", "malignant"], size = 12)
plt.yticks(tick_marks, ["benign", "malignant"], size = 12)
plt.tight_layout()
